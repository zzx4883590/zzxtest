...
def add_case(caseName = "all_Case", rule="test00*.py"):
    """第一步：加载所有的测试用例"""
    case_path = os.path.join(cur_path, caseName)  # 用例文件夹
    # 如果不存在这个case文件夹，就自动创建一个
    if not os.path.exists(case_path): os.mkdir(case_path)
    print("test all_Case path:%s" % case_path)
    # 定义discover方法的参数
...
def run_case(all_case, reportName="report"):
    ...
    runner = HTMLTestRunner_cn.HTMLTestRunner(stream=fp,
                                              verbosity=2,
                                              title=u'UI Automated Test Report：',
...
if __name__ == "__main__":
    ...
    # 邮箱配置
    sender = "zengxj@i2finance.net"
    # psw = "pewyvmuuguznfifj"  # qq邮箱需要授权码
    psw = "Zzxzxj@1314"
    smtp_server = "smtp.exmail.qq.com"
    port = 465
    receiver = "zengxj@i2finance.net"  # 多个时候为list
    # receiver = ["2xxxx@qq.com", "2xxx@qq.com"]
